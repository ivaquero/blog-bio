...
采样法对每个预测器使用相同的训练算法，并在训练集的不同随机子集上对其进行训练。即并行训练多个模型取均值，具有二重随机性（数据采样随机，特征选择随机），泛化程度高。若是又放回的采样，则称袋装（bagging），即自举聚合（bootstrap aggregating）的简称；若是不放回采样，则称粘贴（pasting）。
